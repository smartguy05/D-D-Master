...
// outcome: { output (JSON for the model), rolls[], spawned[], sceneChanged? }
```
...
4. The `output` is small and model-friendly: names, HP strings and dice arrays, with no internal fields.
